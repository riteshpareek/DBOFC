...
(`obf_admin` - a fixed name; every `obf_*` object lives only there, never in the
application schema). Install it **once per MariaDB server**, then obfuscate any number
...
- [ ] Confirm that connected to the **lower env** database, not production
- [ ] The production -> lower data copy is **complete** for the target schema for obfuscation.
- [ ] Take a **snapshot / backup** of the target schema (so a bad run is recoverable
      beyond the resume path).
- [ ] **Disable any triggers ** in the target schema that. The framework does **not** disable
      triggers; they will fire on the obfuscating `UPDATE`s exactly as any normal `UPDATE`
      would.
- [ ] Make sure no activity is hitting the target schema during the run.
...
| `dap_User.UserID` | primary key **and** a stripped, local-part-shaped identifier derived from the user's real email (e.g. `Tom.Hanks` for `Tom.Hanks@sa.gov.au`) - not the email address itself, but still real PII |
...
authored against - a stripped, local-part-shaped value derived from it, e.g. `Tom.Hanks`
for `Tom.Hanks@sa.gov.au`). Either shape works unchanged; the mapping generator and
...
```bash
mariadb -h <host> -P 3306 -u <user> -p < Implementation.sql
```
...
INSERT INTO obf_admin.obf_ObfuscationConfig (TargetSchema, TableName, ColumnName, ObfuscationType, StaticValue) VALUES
  ('appian_db', 'dap_User',  'FirstName',   'FIRST_NAME', NULL),
  ('appian_db', 'dap_User',  'LastName',    'LAST_NAME',  NULL),
  ('appian_db', 'dap_User',  'PhoneNumber', 'PHONE',      NULL),
  ('appian_db', 'dap_User',  'Address',     'ADDRESS',    NULL),
  ('appian_db', 'dap_User',  'SecondaryEmail','EMAIL',    NULL),
  ('appian_db', 'dap_User',  'PasswordHash','STATIC',     '!obfuscated!'),
  ('appian_db', 'dap_User',  'ApiToken',    'HASH',       NULL),
  ('appian_db', 'dap_Actor', 'FirstName',   'FIRST_NAME', NULL),
  ('appian_db', 'dap_Actor', 'LastName',    'LAST_NAME',  NULL);
```
...
```sql
INSERT INTO obf_admin.obf_TableSeedOverride (TargetSchema, TableName, ColumnName) VALUES ('appian_db', 'SomeTable', 'id');
```
...
INSERT INTO obf_admin.obf_ReportingSnapshotExclusion (TargetSchema, TableName, Reason)
  VALUES ('appian_db', 'dap_mv_SomeLiveView', 'Kept in sync by live triggers, not an inert snapshot.');
```
...
```sql
CALL obf_admin.obf_sp_discover_user_references('appian_db', UUID());
```
...
- `TypeLooksCompatible = 0` -> a `WARN`; that column is probably **not** a UserID copy
  ...
  UPDATE obf_admin.obf_UserReferenceRegistry SET Enabled = FALSE
   WHERE TargetSchema = 'appian_db' AND TableName = '...' AND ColumnName = '...';
  ```

**4b. Validate the config against the live schema:**

```sql
CALL obf_admin.obf_sp_validate_config('appian_db', UUID());
```
...
```sql
CALL obf_admin.obf_sp_validate_reference_column_lengths('appian_db', UUID());
```
...
UPDATE obf_admin.obf_UserReferenceRegistry SET OrphanAction = 'IGNORE'
 WHERE TargetSchema = 'appian_db' AND TableName = 'dap_Actor' AND ColumnName = 'CreatedBy';
```
...
CALL obf_admin.obf_sp_obfuscate_database(
    'appian_db',                               -- p_target_schema: the application schema to obfuscate
...
```sql
CALL obf_admin.obf_sp_obfuscation_status('appian_db');
```
...
WHERE TargetSchema = 'appian_db' AND RunID = '<the RunID>'
ORDER BY LogID;
```
...
SELECT COUNT(*) FROM appian_db.dap_User u
LEFT JOIN obf_admin.obf_UserObfuscationMapping m
  ON m.TargetSchema = 'appian_db' AND m.ObfuscatedUserID = CONVERT(u.UserID USING utf8mb4) COLLATE utf8mb4_general_ci
...
FROM appian_db.dap_Actor a
LEFT JOIN obf_admin.obf_UserObfuscationMapping m
  ON m.TargetSchema = 'appian_db' AND m.ObfuscatedUserID = CONVERT(a.CreatedBy USING utf8mb4) COLLATE utf8mb4_general_ci
...
## 9. Lock down the mapping table, then open the environment
...
- **Purge** - no further delta sync planned for this cycle:
  ```sql
  CALL obf_admin.obf_sp_purge_sensitive_staging('appian_db', UUID());
...
```sql
CALL obf_admin.obf_sp_obfuscate_database('appian_db', 'CHANGE-ME-secret-salt-for-this-refresh', 50000, FALSE);
```
...
## 11. Repeat refreshes & housekeeping
...
- Run-history tables (`obf_ObfuscationRun`, `obf_ObfuscationRunLog`, `obf_ObfuscationRowCountSnapshot`)
  ...
  ```sql
  CALL obf_admin.obf_sp_obfuscation_prune('appian_db', 20);   -- keep the newest 20 runs for this target
  ```
